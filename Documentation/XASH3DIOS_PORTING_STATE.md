...
- Candidate commit: this work-order commit; exact commit and workflow/artifact identifiers are recorded after CI without retriggering the build.
...
- Local validation: both attached logs pass their exact analyzer expectations; the Python analyzers compile; the liveness patch reverses and reapplies cleanly over the exact pinned Diffusion tree after the accepted patches; and the applied-source iOS policy validator passes. This Windows worker has no local Apple SDK or compiler, so the required arm64 `XASH_IOS=1` client/server/menu compilation and full IPA contract are performed by GitHub Actions.
- Acceptance state: pending CI and then one Run-42 device test. Build success alone is not device acceptance.
