...
## Phase P production integration and build qualification — Outcome C

- Reconciled baseline: local HEAD, tracking HEAD, and live remote HEAD were clean and equal at `d830684a1d8278c27b76bc5bc7491b50d4f46143`; accepted patch commit `d37bf36a5b707273359728a5ae08f81e712bea5d` remains preserved in ancestry.
- Durable integration: `ddadc3d83c5584fed6e516ab3beaaf727f780122`; accepted patch remains `10,564` bytes / SHA-256 `91AB64B6C392303BEA189BE2D66E409836489DFC6F46F2FC3DFB0BACCFA60FE4`; contract is `scripts/ios/wo56p-shader-lod-compatibility-contract.json`; validator is `scripts/ios/validate-ios-shader-lod-compatibility.py`.
- Production replay: all 13 accepted GL4ES patches applied in canonical order at `81547d986798e876de8b434193920b606a72363f`; the new patch was last, after provider lifecycle; the complete reverse returned to exact pin with a clean worktree.
- Representative positive controls: BmodelSolid/StudioSolid fragments retain 11 core LOD calls, GrassDlight retains 9, rejected extension/EXT tokens remain zero, paired vertices reconvert with `need_essl300=1`, and legacy ESSL 100 output remains unchanged at `937a60863d52d6b5ff7d73aa19baa8ea13c81a8a7cde325193f368860e8cd387`.
- Fingerprint precision: current preserved Diffusion assembly produces GrassDlight vertex source SHA-256 `c101ec6d21d8c42f5b22c306151b6ebb2769e59b5aaca4d57f297a1e3430878b`, correcting the earlier `d61684a3...` source-only record. Baseline conversion remains `f1e4e9c4...` and ESSL 300 reconversion remains `19c1c6ea...`; no behavior conclusion changes.
- Negative/retained controls: omitted/reordered patch, missing family/owner, returned ESSL 300 EXT path, implicit/constant weakening, fabricated capability, source-order corruption, unlinked acceptance, and suppression fixtures fail; drawable, uint, index, topology, transform, texture-unit, material, texture-array, provider, ordinary-bootstrap, self-test, and renderer contract gates pass locally.
- Sole CI: workflow `32616482259`, job `97137964583`, SHA `ddadc3d83c5584fed6e516ab3beaaf727f780122`, `workflow_dispatch`, result `failure`. The new shader-LOD validator passed; the next cumulative Phase G scope check rejected two authorized changed paths before any affected translation-unit/native/package/IPA work ran.
- Mechanical correction: `scripts/ios/gl4es-shaderconv-dump.c` and `scripts/ios/validate-diffusion-mobile-shaders.sh` are added to both cumulative scope allowlists; local self-test boot and renderer-contract mutation suites pass. No duplicate workflow is permitted by this checkpoint.
- Publication absence: GitHub artifacts API reports `total_count=0`; no build-qualified candidate, artifact, IPA, archive digest, tempfile object/link, device request/evidence, crash work, or later phase was produced.
- Outcome: **C**. First unqualified boundary is the full native/package/IPA CI matrix after the corrected cumulative allowlists.
